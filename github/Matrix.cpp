...
static DOUBLE_SPS sqrarg;
#define SQR(a) ((sqrarg=(a)) == 0.0 ? 0.0 : sqrarg*sqrarg)
static DOUBLE_SPS maxarg1, maxarg2;
...
Matrix::Matrix(const int32_t m_, const int32_t n_, const DOUBLE_SPS* val_) {
	allocateMemory(m_, n_);
	int32_t k = 0;
...
Matrix::Matrix(const Matrix &M) {
	...
	for (int32_t i = 0; i<M.m; i++)
		memcpy(val[i], M.val[i], M.n * sizeof(DOUBLE_SPS));
}
...
void Matrix::getData(DOUBLE_SPS* val_, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
	if (i2 == -1) i2 = m - 1;
	if (j2 == -1) j2 = n - 1;
	int32_t k = 0;
...
void Matrix::setVal(DOUBLE_SPS s, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
	if (i2 == -1) i2 = m - 1;
	if (j2 == -1) j2 = n - 1;
	if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(DOUBLE_SPS s, int32_t i1, int32_t i2) {
...
Matrix Matrix::rotMatX(const DOUBLE_SPS &angle) {
	DOUBLE_SPS s = sin(angle);
	DOUBLE_SPS c = cos(angle);
	Matrix R(3, 3);
	R.val[0][0] = +1;
	R.val[1][1] = +c;
	R.val[1][2] = -s;
...
Matrix Matrix::rotMatY(const DOUBLE_SPS &angle) {
	DOUBLE_SPS s = sin(angle);
	DOUBLE_SPS c = cos(angle);
	Matrix R(3, 3);
	R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ(const DOUBLE_SPS &angle) {
	DOUBLE_SPS s = sin(angle);
	DOUBLE_SPS c = cos(angle);
	Matrix R(3, 3);
	R.val[0][0] = +c;
...
Matrix Matrix::operator* (const DOUBLE_SPS &s) {
	Matrix C(m, n);
	for (int32_t i = 0; i<m; i++)
		for (int32_t j = 0; j<n; j++)
			C.val[i][j] = val[i][j] * s;
...
DOUBLE_SPS Matrix::mean() {
	DOUBLE_SPS mean = 0;
	for (int32_t i = 0; i<m; i++)
		for (int32_t j = 0; j<n; j++)
			mean += val[i][j];
	return mean / (DOUBLE_SPS)(m*n);
}
...
bool Matrix::solve(const Matrix &M, DOUBLE_SPS eps) {
	...
	DOUBLE_SPS big, dum, pivinv, temp;
...
bool Matrix::lu(int32_t *idx, DOUBLE_SPS &d, DOUBLE_SPS eps) {
	...
	int32_t i, imax, j, k;
	DOUBLE_SPS   big, dum, sum, temp;
	DOUBLE_SPS* vv = (DOUBLE_SPS*)malloc(n * sizeof(DOUBLE_SPS)); // vv stores the implicit scaling of each row.
	d = 1.0;
...
// U?¡èW?¡èV T. Thematrix U replaces a on output. The diagonal matrix of singular values W is output
...
void Matrix::svd(Matrix &U2, Matrix &W, Matrix &V) {
	...
	DOUBLE_SPS* w = (DOUBLE_SPS*)malloc(n * sizeof(DOUBLE_SPS));
	DOUBLE_SPS* rv1 = (DOUBLE_SPS*)malloc(n * sizeof(DOUBLE_SPS));

	int32_t flag, i, its, j, jj, k, l, nm;
	DOUBLE_SPS   anorm, c, f, g, h, s, scale, x, y, z;
	...
	for (k = n - 1; k >= 0; k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
		for (its = 0; its<30; its++) { // and over allowed iterations.
			...
			for (l = k; l >= 0; l--) { // Test for splitting.
				nm = l - 1;
				if ((DOUBLE_SPS)(fabs(rv1[l]) + anorm) == anorm) { flag = 0; break; }
				if ((DOUBLE_SPS)(fabs(w[nm]) + anorm) == anorm) { break; }
			}
			if (flag) {
				...
				for (i = l; i <= k; i++) {
					f = s*rv1[i];
					rv1[i] = c*rv1[i];
					if ((DOUBLE_SPS)(fabs(f) + anorm) == anorm) break;
					g = w[i];
					h = pythag(f, g);
					w[i] = h;
					h = 1.0 / h;
	...
	int32_t s2, inc = 1;
	DOUBLE_SPS   sw;
	DOUBLE_SPS* su = (DOUBLE_SPS*)malloc(m * sizeof(DOUBLE_SPS));
	DOUBLE_SPS* sv = (DOUBLE_SPS*)malloc(n * sizeof(DOUBLE_SPS));
...
void Matrix::allocateMemory(const int32_t m_, const int32_t n_) {
	...
	val = (DOUBLE_SPS**)malloc(m * sizeof(DOUBLE_SPS*));
	val[0] = (DOUBLE_SPS*)calloc(m*n, sizeof(DOUBLE_SPS));
...
DOUBLE_SPS Matrix::pythag(DOUBLE_SPS a, DOUBLE_SPS b) {
	DOUBLE_SPS absa, absb;
	absa = fabs(a);
	absb = fabs(b);
	if (absa > absb)
