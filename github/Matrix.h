...
typedef double DOUBLE_SPS;      // double precision
								//typedef float  DOUBLE_SPS;    // single precision
...
public:
	...
	Matrix(const int32_t m, const int32_t n, const DOUBLE_SPS* val_); // init mxn matrix with values from array 'val'
	...
	void getData(DOUBLE_SPS* val_, int32_t i1 = 0, int32_t j1 = 0, int32_t i2 = -1, int32_t j2 = -1);
	...
	void setVal(DOUBLE_SPS s, int32_t i1 = 0, int32_t j1 = 0, int32_t i2 = -1, int32_t j2 = -1);

	// set (part of) diagonal to scalar, -1 as end replaces whole diagonal
	void setDiag(DOUBLE_SPS s, int32_t i1 = 0, int32_t i2 = -1);
	...
	static Matrix rotMatX(const DOUBLE_SPS &angle);
	static Matrix rotMatY(const DOUBLE_SPS &angle);
	static Matrix rotMatZ(const DOUBLE_SPS &angle);
	...
	Matrix  operator* (const DOUBLE_SPS &s);  // multiply with scalar
	Matrix  operator/ (const Matrix &M); // divide elementwise by matrix (or vector)
	Matrix  operator/ (const DOUBLE_SPS &s);  // divide by scalar
	Matrix  operator- ();                // negative matrix
	Matrix  operator~ ();                // transpose
	DOUBLE_SPS   l2norm();                   // euclidean norm (vectors) / frobenius norm (matrices)
	DOUBLE_SPS   mean();                     // mean of all elements in matrix
	...
	DOUBLE_SPS  det();                                             // returns determinant of matrix
	bool   solve(const Matrix &M, DOUBLE_SPS eps = 1e-20);            // solve linear system M*x=B, replaces *this and M
	bool   lu(int32_t *idx, DOUBLE_SPS &d, DOUBLE_SPS eps = 1e-20);        // replace *this by lower upper decomposition
...
private:
	...
	void releaseMemory();
	inline DOUBLE_SPS pythag(DOUBLE_SPS a, DOUBLE_SPS b);
