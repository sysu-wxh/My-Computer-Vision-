...
class SEEDS {
public:
	SEEDS(int width, int height, int nr_channels, int nr_bins, float disparity_factor);
	~SEEDS();
	...
	//set disparity image
	void update_disparityImage(IplImage* disparityImage);
	...
	void SaveParentLabels_Text(string filename, int level);
	// mean colors
	void compute_mean_map();
	UINT* means;
	float lambda;

	void update_blocks_d(double** planeFunction);

	// 输出parent label
	void getParrentLabel(int level, UINT* parentLabel);
	// extract vertex after plane extraction
	void findcontour();
	void planeCalculateVertex(std::vector<VERTEX>& result);
private:
	void deinitialize();
	...
	void compute_means();
	void lab_get_histogram_cutoff_values(const UINT* image);
	...
	float calculateDisparityDifference(int level1, int label1, int level2, int label2);
